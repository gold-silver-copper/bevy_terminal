...
impl ratatui::backend::Backend for BevyTerminalBackend {
    ...
    fn scroll_region_up(
        ...
        self.cells
            .resize(bounds.height as usize * bounds.width as usize, Cell::EMPTY);
        let start = region.start.min(bounds.height);
        let end = region.end.min(bounds.height).max(start);
        let count = line_count.min(end - start);
        let width = bounds.width;
        for y in start..end.saturating_sub(count) {
            for x in 0..width {
                self.cells[(y * width + x) as usize] =
                    self.cells[((y + count) * width + x) as usize].clone();
            }
        }
        for y in end.saturating_sub(count)..end {
            for x in 0..width {
                self.cells[(y * width + x) as usize].reset();
            }
        }
        self.dirty_rows.resize(bounds.height as usize, true);
        self.dirty_rows.fill(true);
        self.cells_changed_last_draw = count > 0;
        self.full_redraw_needed |= count > 0;
        Ok(())
    }
